...
## 0.2 Pick the Gemini Enterprise edition you'll target

This affects what features you can demo at the end. Pick now so you don't get surprised in section 12.
...
Most developer steps use Google Cloud services directly. Section 12 needs an existing Gemini Enterprise app and the right admin permissions.

## 0.3 Required IAM roles on the project
...
- `roles/discoveryengine.admin` — for Gemini Enterprise app agent registration in section 12
- `roles/resourcemanager.projectCreator` — only if you'll create projects yourself

## 0.4 Install local software

### 0.4.1 Python 3.10 or later
...
### 0.4.4 Docker (optional, only for GKE / custom containers)
...
### 0.4.5 Visual Studio Code (recommended editor)

1. Download from https://code.visualstudio.com/.
2. Install the **Python**, **Pylance**, and **Google Cloud Code** extensions.

## 0.5 Sign in to gcloud and set Application Default Credentials
...
## 0.6 PowerShell execution policy (Windows only)
...
When all boxes are checked, move on to **`01_project_setup.md`**.
